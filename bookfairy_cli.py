...
"""
BookFairy CLI: Fetch SFPL availability candidate and Goodreads rating for a list of books.
...
Output:
  Prints each matched title with its Goodreads rating (or N/A if not found).
...
# Ensure we can import the Django-app modules when running as a standalone script
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if SCRIPT_DIR not in sys.path:
    sys.path.insert(0, SCRIPT_DIR)

try:
    # Local modules from the existing app
    from bookapp.library import get_library_details
    from bookapp.goodreads import find_gr_rating
except Exception as e:
    print("Error importing project modules. Ensure you run this from the repo root (BookFairy).")
...
def extract_spreadsheet_key(gsheet: str) -> Optional[str]:
    """If a Google Sheets URL is given, extract the spreadsheet key.
    Returns None if not a URL or cannot extract.
    """
...
def load_books_from_gspread(spreadsheet: str, worksheet: Optional[str]) -> List[Tuple[str, str]]:
    ...
    - spreadsheet: title, key, or full URL
    - worksheet: optional worksheet (tab) name. If not provided, tries "To Read" then first sheet
    """
    try:
        import gspread
    except Exception as e:
        print("gspread is required for --gspread input. Install with 'pip install gspread google-auth'.")
        raise

    # Use browser OAuth (for end users). Requires credentials.json present locally (once).
    ...
    if worksheet:
        ...
        # Prefer a worksheet named "To Read" if present; otherwise first worksheet
        try:
    ...
    books: List[Tuple[str, str]] = []
    # Flexible parsing: either two columns (Title, Author) or one column "Title, Author"
    for r in rows:
        if not r:
            continue
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    group.add_argument("--file", dest="file_path", help="Path to text file with 'Title, Author' per line")
    group.add_argument("--text", dest="raw_text", help="Raw text with 'Title, Author' per line")
    group.add_argument("--gspread", dest="gspread_ref", help="Google Sheet title, key, or URL (OAuth in browser)")

    parser.add_argument("--worksheet", help="Worksheet name for Google Sheet (default: 'To Read' if exists)")
    parser.add_argument("--lib-location", default="3", help="SFPL location code (default: '3' MAIN)")
