...
from bookie.forms import RegistrationForm, LoginForm
from bookie.models import Bookie

user = User.objects.create_user('michelleglauser', 'michelleglauser@gmail.com', 'password')


def BookieRegistration(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # bookie = user.get_profile()
            # bookie.name = form.cleaned_data['name']
            # bookie.booklist = form.cleaned_data['booklist']
            # bookie.save()
            # user = requests.user.get_profile()
            bookie = Bookie(user=user, name=form.cleaned_data['name'], birthday=form.cleaned_data['birthday'])
            bookie.save()
            return HttpResponseRedirect('/profile/')
        else:
...
def LoginRequest(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid();
            ...
            bookie - authenticate(username=username, password=password)
            if bookie is not None:
                login(request, bookie)
                return HttpResponseRedirect('/profile/')
            else:
                return HttpResponseRedirect('/login/')
        else: 
            return render_to_response('login.html', {'form': form}, context_instance=RequestContext)
